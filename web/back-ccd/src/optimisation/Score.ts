import { BoxWithArticle } from "./Temp";
import { Campaign } from "../entities/Campaign";
import { Usertochild } from "../entities/UserToChild";
import { State } from "../entities/enums/State";

const INVALID_SCORE = -9999999999999;

type ScoreResult = {
  score: number;
  perBoxScore: Map<string, number>;
};

export default class Score {
  usedArticles = new Set<string>();

  evaluateComposition(
    boxes: Array<BoxWithArticle>,
    campaign: Campaign,
    usersToChild: Array<Usertochild>,
  ): ScoreResult {
    this.usedArticles = new Set<string>();
    let totalScore = 0;
    let perBoxScore = new Map<string, number>();

    for (const box of boxes) {
      // Reset box metrics before evaluation
      box.box.score_box = 0;
      box.box.total_weight = 0;
      box.box.total_price = 0;

      // Règle 7 : Tout le monde est servi
      if (box.articles.length === 0) {
        totalScore -= 10;
        box.box.score_box = -10;
      } else {
        let userToChild = usersToChild.find(
          (utc) => utc.id_user === box.box.id_user,
        );
        if (userToChild) {
          let boxMetrics = this.evaluateBox(box, campaign, userToChild);
          if (boxMetrics.score === INVALID_SCORE) {
            return {
              score: 0,
              perBoxScore: new Map<string, number>(),
            };
          }

          // Store metrics in the box entity
          box.box.score_box = boxMetrics.score;
          box.box.total_weight = boxMetrics.weight;
          box.box.total_price = boxMetrics.price;

          perBoxScore.set(box.box.id_box, boxMetrics.score);
          totalScore += boxMetrics.score;
        }
      }

      // Règle 8 : Equité (Note: this is a global rule, not per-box, so we apply it to totalScore)
      for (const otherBox of boxes) {
        if (box.box.id_box !== otherBox.box.id_box) {
          if (otherBox.articles.length - box.articles.length >= 2) {
            totalScore -= 10;
            box.box.score_box -= 10;
            break;
          }
        }
      }
    }

    return {
      score: totalScore,
      perBoxScore: perBoxScore,
    };
  }

  evaluateBox(
    box: BoxWithArticle,
    campaign: Campaign,
    child: Usertochild,
  ): { score: number; weight: number; price: number } {
    let totalWeight = 0;
    let totalPrice = 0;
    let score = 0;
    let categoriesCount = new Map<string, number>();

    for (const article of box.articles) {
      // Règle 1 : Unicité des articles
      if (this.usedArticles.has(article.id_article)) {
        return { score: INVALID_SCORE, weight: 0, price: 0 };
      }
      this.usedArticles.add(article.id_article);

      // Règle 2 : Compatibilité d'âge
      if (article.age_range !== child.age_range) {
        return { score: INVALID_SCORE, weight: 0, price: 0 };
      }

      // Règle 3 : Poids limité
      totalWeight += article.weight;
      totalPrice += article.price;
      if (totalWeight > campaign.max_weight) {
        return { score: INVALID_SCORE, weight: 0, price: 0 };
      }

      // Règle 4 : Gain par préférence de catégorie
      let preferences = child.preference;
      let idx = preferences.findIndex((pref) => pref === article.category);

      // Règle 6 : Utilités dégressives
      if (!categoriesCount.has(article.category)) {
        categoriesCount.set(article.category, 1);
      } else {
        idx += categoriesCount.get(article.category) || 0;
        categoriesCount.set(
          article.category,
          (categoriesCount.get(article.category) || 0) + 1,
        );
      }

      switch (idx) {
        case 0:
          score += 10;
          break;
        case 1:
          score += 8;
          break;
        case 2:
          score += 6;
          break;
        case 3:
          score += 4;
          break;
        case 4:
          score += 2;
          break;
        default:
          score += 1;
          break;
      }

      // Règle 5 : Bonus d'état
      switch (article.state) {
        case State.N.code:
          score += 2;
          break;
        case State.TB.code:
          score += 1;
          break;
        default:
          break;
      }
    }
    return { score, weight: totalWeight, price: totalPrice };
  }
}
