...
export default class Glouton {
  ...
  optimize(
    ...
    let currentScore = this.scoreService.evaluateComposition(
      boxes,
      campaign,
      usersToChild,
    ).score;
    ...
    while (improved) {
      ...
      for (const article of remainingArticles) {
        for (const box of boxes) {
          ...
          const newScore = this.scoreService.evaluateComposition(
            boxes,
            campaign,
            usersToChild,
          ).score;
      ...
      if (bestArticle && bestBox && bestGain > 0) {
        ...
        currentScore = this.scoreService.evaluateComposition(
          boxes,
          campaign,
          usersToChild,
        ).score;
