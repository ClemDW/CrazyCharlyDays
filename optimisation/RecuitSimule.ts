  ...
  optimize(
    ...
    const tryMove = (boxes: BoxWithArticle[], rem: Set<string>): boolean => {
      ...
      for (let attempt = 0; attempt < targetIndices.length; attempt++) {
        ...
        const userTarget = usersToChild.find(
          (u) => u.id_user === targetBox.box.id_user,
        );
        if (!userTarget) continue;
        if (article.age_range !== userTarget.age_range) continue;

        const targetWeight = targetBox.articles.reduce(
          (s, it) => s + it.weight,
          0,
        );
    ...
    const trySwap = (boxes: BoxWithArticle[], rem: Set<string>): boolean => {
      ...
      let tries = 0;
      while (j === i && tries++ < 10)
        j = filledIndices[randInt(filledIndices.length)];
      if (i === j) return false;
      ...
      const weightA =
        boxA.articles.reduce((s, it) => s + it.weight, 0) -
        artA.weight +
        artB.weight;
      const weightB =
        boxB.articles.reduce((s, it) => s + it.weight, 0) -
        artB.weight +
        artA.weight;
      if (weightA > campaign.max_weight || weightB > campaign.max_weight)
        return false;
    ...
    while (
      T > this.opts.minTemperature &&
      iterations < this.opts.maxIterations
    ) {
